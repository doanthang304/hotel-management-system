...
const PAST_DAYS = 30;         // Số ngày trong quá khứ
...
export function BookingCalendar() {
  ...
  // View state: Fixed timeline
  const [baseDate] = useState<Date>(() => startOfDay(new Date()));
  const startDate = useMemo(() => {
    return addDays(baseDate, -PAST_DAYS);
  }, [baseDate]);
  const endDate = useMemo(() => {
    return addDays(baseDate, NUM_DAYS - PAST_DAYS - 1);
  }, [baseDate]);
  ...
  // Để màn hình vừa đúng 7 ngày, tổng chiều rộng sẽ là (NUM_DAYS / 7) * 100%
    ...
    if (scrollRef.current) {
      ...
      if (todayEl) {
        scrollRef.current.scrollTo({
          left: (todayEl as HTMLElement).offsetLeft,
  ...
  }, [loading, rooms.length, scrollToToday]);
  ...
  // ── Event helpers ──────────────────────────────────────────────────────────
  /**
   * Tính toán % vị trí left và width của booking bar trên tổng timeline (NUM_DAYS)
   */
  function getBarGeometry(event: GanttEvent) {
    ...
    const clampedRight = Math.min(rightEdgeDays, NUM_DAYS);
    const widthDays = clampedRight - clampedLeft;

    if (widthDays <= 0) {
      return { left: "0%", width: `calc(${(0.5 / NUM_DAYS) * 100}% - 4px)` };
    }

    return {
      left: `calc(${(clampedLeft / NUM_DAYS) * 100}% + 2px)`,
      width: `calc(${(widthDays / NUM_DAYS) * 100}% - 4px)`,
    };
  }
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-1.5">
          <Button variant="outline" size="icon-sm" onClick={goPrev} title="Trang trước">
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="sm" onClick={() => scrollToToday(true)} className="gap-1.5">
            <CalendarDays className="h-3.5 w-3.5" />
            Hôm nay
          </Button>
          <Button variant="outline" size="icon-sm" onClick={goNext} title="Trang sau">
      ...
      {/* Gantt Grid */}
      <div className="relative rounded-lg border bg-card shadow-sm overflow-hidden flex flex-col">
        {loading && (
        ...
        <div className="flex flex-1">
          ...
          >
            <div style={{ width: `${totalWidthPercent}%`, minWidth: "100%" }}>
              {/* Day header row */}
              <div
              ...
              >
                {days.map((day) => (
                  <div
                    ...
                    style={{ width: `${dayWidthPercent}%`, scrollSnapAlign: "start" }}
                  >
                    {/* Divider */}
              ...
              {!loading && rooms.length === 0 && (
                <div className="flex h-40 items-center justify-center text-muted-foreground text-sm w-full absolute left-0">
          ...
          <div className="rounded-lg border bg-popover px-3 py-2 shadow-lg text-xs space-y-1 min-w-[160px]">
            ...
            {tooltip.event.source && (
              <p className="text-muted-foreground text-[11px]">
                Nguồn: <span className="font-medium text-foreground">{tooltip.event.source}</span>
              </p>
            )}
            {/* ---------------------------------- */}
            <span className={cn(
